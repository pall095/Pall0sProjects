...
// every call to export_r assumes that the node has been already labelled in a previous call
void export_r(FILE *fout, node_t *node) {
    if( node->left != NULL ) {
        char *dst_name = toString(node->left->val);
        fprintf(fout, "\"%p\" [label=\"%s\"];\n",(void *)(node->left), dst_name);
        fprintf(fout, "\"%p\" -> \"%p\";\n",(void *)node,(void *)node->left);
        export_r(fout, node->left);
    }
    if( node->right != NULL ) {
        char *dst_name = toString(node->right->val);
        fprintf(fout, "\"%p\" [label=\"%s\"];\n",(void *)(node->right), dst_name);
        fprintf(fout, "\"%p\" -> \"%p\";\n",(void *)node,(void *)node->right);
        export_r(fout, node->right);
    }
}

void export(FILE *fout, node_t *node) {
    fprintf(fout,"digraph {\n");
    fprintf(fout, "\"%p\" [label=\"%s\"];\n",(void *)node, toString(node->val));
    export_r(fout, node);
    fprintf(fout, "}\n");
}
